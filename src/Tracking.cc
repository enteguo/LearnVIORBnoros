...
void Tracking::PredictNavStateByIMU(bool bMapUpdated)
{
    ...
    if (mpLocalMapper->GetFirstVINSInited() || bMapUpdated)
    {
        ...
        mCurrentFrame.UpdatePoseFromNS(ConfigParam::GetMatTbc()); //转化到摄像机坐标系下
...
bool Tracking::TrackWithIMU(bool bMapUpdated)//替换了原来的 TrackWithMotionModel();
{
    ...
    //计算噪声协方差，雅可比矩阵，两帧之间相对pose，当前帧pose
...
IMUPreintegrator Tracking::GetIMUPreIntSinceLastKF(Frame* pCurF, KeyFrame* pLastKF, const std::vector<IMUData>& vIMUSInceLastKF)
{
    ...
    // remember to consider the gap between the last KF and the first IMU
    {
        ...
        IMUPreInt.update(imu._g - bg, imu._a - ba, dt);  //更新雅可比矩阵和噪声协方差矩阵传播，两帧之间的运动
    ...
    for (size_t i = 0; i < vIMUSInceLastKF.size(); i++)
    {
        ...
        if (i == vIMUSInceLastKF.size() - 1)
            nextt = pCurF->mTimeStamp;         // last IMU, next is this KeyFrame，把当前帧的时间作为IMU时间
        else
...
{
    mvIMUSinceLastKF.insert(mvIMUSinceLastKF.end(), vimu.begin(), vimu.end());    //插入IMU测量值
    mImGray = im;
...
//传入图像，时间戳，imu数据，特征点提取器，字典，内参矩阵等参数来构造关键帧
    if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET)  //mThDepth与单目无关
...
/****************************************************************************/
    ...
    int nFeatures = fSettings["ORBextractor.nFeatures"];   //特征点个数，1000
    float fScaleFactor = fSettings["ORBextractor.scaleFactor"];   //金字塔倍数，1.2
    int nLevels = fSettings["ORBextractor.nLevels"];             //金字塔层数，8
    int fIniThFAST = fSettings["ORBextractor.iniThFAST"];        //初始Fast特征点响应
    int fMinThFAST = fSettings["ORBextractor.minThFAST"];        //如果没提取到，则采用最小特征点响应

    mpORBextractorLeft = new ORBextractor(nFeatures, fScaleFactor, nLevels, fIniThFAST, fMinThFAST);//初始化，每一层大小，特征点个数

    if (sensor == System::STEREO)
        mpORBextractorRight = new ORBextractor(nFeatures, fScaleFactor, nLevels, fIniThFAST, fMinThFAST);
//单目初始化时所用，只是比 mpORBextractorLeft多一倍特征点tracking
...
//重新计算偏置
        ...
        {
            // Update motion model
            //计算lastframe到当前帧的运动，将其作为运动模型
            if (!mLastFrame.mTcw.empty())
            {
                cv::Mat LastTwc = cv::Mat::eye(4, 4, CV_32F);
                mLastFrame.GetRotationInverse().copyTo(LastTwc.rowRange(0, 3).colRange(0, 3));
                mLastFrame.GetCameraCenter().copyTo(LastTwc.rowRange(0, 3).col(3));
                mVelocity = mCurrentFrame.mTcw * LastTwc;  //前一帧到当前帧的位姿变化Tcl；
            }
            else
                mVelocity = cv::Mat();
...
void Tracking::MonocularInitialization()
{
    ...
    if (!mpInitializer)
    {
        ...
        if (mCurrentFrame.mvKeys.size() > 100)
        {
            ...
            mLastFrame = Frame(mCurrentFrame);
            mvbPrevMatched.resize(mCurrentFrame.mvKeysUn.size()); //存储了另一帧上匹配的特征点坐标
            ...
            fill(mvIniMatches.begin(), mvIniMatches.end(), -1);   //类似于memset，匹配上另一帧上特征点的ID序号，nMatches12
    ...
    else
    {
        ...
        ORBmatcher matcher(0.9, true);
        //在windows范围内搜索匹配点
        int nmatches = matcher.SearchForInitialization(mInitialFrame, mCurrentFrame, mvbPrevMatched, mvIniMatches, 100); //返回匹配的个数
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // Create KeyFrames
    KeyFrame* pKFini = new KeyFrame(mInitialFrame, mpMap, mpKeyFrameDB, vimu1, NULL);  //第一帧
    pKFini->ComputePreInt();
    KeyFrame* pKFcur = new KeyFrame(mCurrentFrame, mpMap, mpKeyFrameDB, vimu2, pKFini); //初始化的当前帧
    ...
    Optimizer::GlobalBundleAdjustemnt(mpMap, 20);   //优化

    // Set median depth to 1
    float medianDepth = pKFini->ComputeSceneMedianDepth(2);  //3D点深度的中值
    float invMedianDepth = 1.0f / medianDepth;

    if (medianDepth < 0 || pKFcur->TrackedMapPoints(1) < 100)   //初始化成功与否的判断条件，特征点点数小于100，深度小于0
    {
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    vector<MapPoint*> vpMapPointMatches; //当前帧的匹配三维点
    ...
    mCurrentFrame.SetPose(mLastFrame.mTcw);  //此处位姿可以直接用IMU计算出来的
...
bool Tracking::TrackWithMotionModel()
{
    ...
    UpdateLastFrame();   //参考帧的pose

    mCurrentFrame.SetPose(mVelocity * mLastFrame.mTcw);  //此处个人认为mVelocity应该用IMU计算
...
bool Tracking::TrackLocalMap()
{
    ...
    UpdateLocalMap();  //当前帧localKF和localMapPoints加入

    SearchLocalPoints();  //是否在当前帧可见，通过投影匹配

    // Optimize Pose
    Optimizer::PoseOptimization(&mCurrentFrame);  //优化pose
    mnMatchesInliers = 0;

    // Update MapPoints Statistics
    //当前帧的MapPoints被多少其他帧观测到
    for (int i = 0; i < mCurrentFrame.N; i++)
    {
        if (mCurrentFrame.mvpMapPoints[i])
        {
...
void Tracking::SearchLocalPoints()
{
    ...
    for (vector<MapPoint*>::iterator vit = mvpLocalMapPoints.begin(), vend = mvpLocalMapPoints.end(); vit != vend; vit++)
    {
        ...
        if (mCurrentFrame.isInFrustum(pMP, 0.5))  //是否在视野内，三种策略
        {
    ...
    if (nToMatch > 0)
    {
        ...
        if (mCurrentFrame.mnId < mnLastRelocFrameId + 2)
            th = 5;
        matcher.SearchByProjection(mCurrentFrame, mvpLocalMapPoints, th); //通过投影搜索
    }
}
...
void Tracking::UpdateLocalMap()
{
    ...
    // Update
    UpdateLocalKeyFrames();  //localKF的加入策略
    UpdateLocalPoints();
}
...
{
//加添局部关键帧的策略：
//1.观测到当前帧的MapPoint的关键帧
//2.观测到当前帧的MapPoint的关键帧中的共视度较高的共视帧
//3.观测到当前帧的MapPoint的关键帧的子关键帧
//4.观测到当前帧的MapPoint的关键帧的父关键帧
//5.更新当前帧的参考关键帧，与自己共视程度最高的关键帧作为参考关键帧


    // Each map point vote for the keyframes in which it has been observed
    map<KeyFrame*, int> keyframeCounter;
    for (int i = 0; i < mCurrentFrame.N; i++)  //遍历当前帧三维关键点
    {
        if (mCurrentFrame.mvpMapPoints[i])
        {
            MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];
            if (!pMP->isBad())
            {
                const map<KeyFrame*, size_t> observations = pMP->GetObservations();  //观测到该关键点的关键帧
                for (map<KeyFrame*, size_t>::const_iterator it = observations.begin(), itend = observations.end(); it != itend; it++)
                    keyframeCounter[it->first]++;   //所有关键帧上可以观测到的关键点数
            }
            else
            {
    ...
    for (map<KeyFrame*, int>::const_iterator it = keyframeCounter.begin(), itEnd = keyframeCounter.end(); it != itEnd; it++)
    {
        ...
        {
            max = it->second;
            pKFmax = pKF;   //观测到关键点数最多的帧
        }

        mvpLocalKeyFrames.push_back(it->first);   //把有观测到三维关键点的关键帧加入localKF
    ...
    for (vector<KeyFrame*>::const_iterator itKF = mvpLocalKeyFrames.begin(), itEndKF = mvpLocalKeyFrames.end(); itKF != itEndKF; itKF++)
    {
        ...
        if (mvpLocalKeyFrames.size() > 80)  //限制localKF中关键帧数量
            break;

        KeyFrame* pKF = *itKF;

        const vector<KeyFrame*> vNeighs = pKF->GetBestCovisibilityKeyFrames(10);  //返回N个按共视排序的关键帧
