      ...
      mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    // ORB extraction
    ExtractORB(0, imGray);  //提取特征点，划分金字塔裁剪划分格子，提取特征点
    ...
    if (mbInitialComputations)
    {
        ...
        mfGridElementWidthInv = static_cast<float>(FRAME_GRID_COLS) / static_cast<float>(mnMaxX - mnMinX);  //格子的个数的倒数
    ...
    AssignFeaturesToGrid();  //为grid赋值，把特征点分配到gird中
}
...
vector<size_t> Frame::GetFeaturesInArea(const float &x, const float  &y, const float  &r, const int minLevel, const int maxLevel) const
{
    ...
    const int nMinCellX = max(0, (int)floor((x - mnMinX - r) * mfGridElementWidthInv));   //x方向属于第几个格子
    ...
    const int nMinCellY = max(0, (int)floor((y - mnMinY - r) * mfGridElementHeightInv));   //y方向属于第几个格子
    ...
    //遍历所有符合的格子，把所有这些格子中的特征点与当前特征点像素位置距离在r以内的点的ID序号输出
